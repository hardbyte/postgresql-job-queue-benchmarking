...
## Q1 — Throughput ceiling: YES, competitive
...
## Q2 — Pin-immunity: YES (dead tuples stay flat)
...
**Dead tuples stay flat through the pin** — the 1,527 peak is *entirely* `seg.ring_state`
(the single mutable rotation pointer); every append-only table (`segments`,
`events_*`, `claims_*`, `done_*`) is **0**. This is the thesis confirmed:
append-only + `TRUNCATE`-on-rotation means a held MVCC horizon causes no
dead-tuple accumulation (vs queue_storage's 130k on `queue_claim_heads`).
...
Net: pin-immunity is **proven**; sustained high throughput is **not yet
demonstrated** and is an engineering + clean-host-measurement item for the RFC.
...
**Open the v0.7 segment-storage RFC, with this spike as the feasibility
evidence.** The spike de-risks the two things the original #169 spike left open:
the storage shape is pin-immune (Q2) AND a cursor allocator makes the claim path
fast enough to be in awa's throughput league while keeping per-job semantics
(Q1). What remains is genuinely design-first work — the semantics contract (Q3)
and the maturity gaps — which is exactly what an RFC should settle before
implementation. Nothing here changes the 0.6 decision: this stays post-0.6,
opt-in, design-first.
